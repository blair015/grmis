...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    $standardRatio = $_POST['standardRatio']; // Updated variable name
    $classroomRatio = $_POST['classroomRatio']; // Updated variable name
    $receivedPackages = $_POST['receivedPackages']; // Updated variable name
    ...
    if ($result->num_rows > 0) {
        ...
        echo '<script>
            ...
            if (confirmOverwrite) {
                window.location = "?school_id=' . $schoolId . '&confirm_overwrite=1&standardRatio=' . urlencode($standardRatio) . '&classroomRatio=' . urlencode($classroomRatio) . '&receivedPackages=' . urlencode($receivedPackages) . '&quarter=' . $quarter . '&schoolYear=' . urlencode($schoolYear) . '";
            } else {
        ...
        insertData($conn, $schoolId, $standardRatio, $classroomRatio, $receivedPackages, $quarter, $schoolYear, $user_school_id);
    }
} elseif (isset($_GET['confirm_overwrite']) && $_GET['confirm_overwrite'] === "1") {
    // User confirmed overwrite, proceed with insertion
    $schoolId = $_GET['school_id'];
    $standardRatio = urldecode($_GET['standardRatio']);
    $classroomRatio = urldecode($_GET['classroomRatio']);
    $receivedPackages = urldecode($_GET['receivedPackages']);
